...
const packageFit: Record<string, string> = {
  ...
  scale: 'Best for large sales pipelines and broad campaigns.',
};
    ...
    <>
      ...
      <main>
        <section className="container hero">
          ...
          <div className="mock">
            ...
            <div className="lead-table" aria-label="Example cleaned lead columns">
              <div className="lead-table-head"><span>Name</span><span>Role</span><span>Email</span></div>
              <div className="lead-table-row"><span>Sarah M.</span><span>Marketing Manager</span><span>verified</span></div>
              <div className="lead-table-row"><span>David K.</span><span>Sales Director</span><span>verified</span></div>
              <div className="lead-table-row"><span>Amanda R.</span><span>Operations Lead</span><span>verified</span></div>
            </div>
          </div>
        </section>
        ...
        <section className="container deliverables">
          ...
          <div className="column-cloud">{leadColumns.map((column) => <span key={column}>{column}</span>)}</div>
        </section>
        ...
        <section id="pricing" className="container">
          ...
          <div className="pricing-grid" style={{ marginTop: 24 }}>
            {packages.map((item) => (
              <div key={item.id} className={`card pricing-card ${item.recommended ? 'featured' : ''}`}>
                <span className="pill">{item.name}</span>
                <div className="price">${item.price}</div>
                <h3>{item.leads.toLocaleString('en-US')} leads</h3>
                <p className="muted">{packageFit[item.id]}</p>
                <div className="list">
                ...
                <Link className="btn btn-primary" href={`/checkout?package=${item.id}`} style={{ width: '100%' }}>Buy package</Link>
              </div>
            ))}
          </div>
        </section>
