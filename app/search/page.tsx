...
export default async function Search({ searchParams }: { searchParams: Promise<{ order?: string }> }) {
  ...
  return <DashboardShell active="New Search">
    ...
    <SearchBuilder order={{ id: order.id, orderCode: order.order_code, leadCount: remaining, packageName: order.packages?.name ?? 'Lead package', deliveryEmail: order.delivery_email }} />
  </DashboardShell>;
}
