...
export function rowsToCsv(rows: LeadRow[]) {
...
function worksheetXml(rows: LeadRow[]) {
  const headers = Object.keys(rows[0] ?? {});
...
function localFileHeader(fileName: string, content: Buffer, crc: number) {
...
function centralDirectoryHeader(fileName: string, content: Buffer, crc: number, offset: number) {
  const name = Buffer.from(fileName);
  const { dosTime, dosDate } = dosDateTime();
...
export function rowsToXlsx(rows: LeadRow[]) {
  const files = [
  ...
  for (const [fileName, fileContent] of files) {
    ...
    const local = localFileHeader(fileName, content, crc);
    localParts.push(local);
    centralParts.push(centralDirectoryHeader(fileName, content, crc, offset));
    offset += local.length;
  }
